...
2. settings.yaml（可通过 IMAGE2PSD_CONFIG 指定）
3. 环境变量覆盖
"""
...
_LOCAL_PATH = Path(os.environ.get("IMAGE2PSD_CONFIG", str(PROJECT_ROOT / "settings.yaml")))
...
def _build_settings() -> "Settings":
    ...
    port = _from_env("PORT", _section_get(c, "server", "port", 9003), int)
    ...
    service_name = str(_section_get(c, "service", "name", "image2psd"))
    ...
    workflow_input_node_id = str(_from_env("COMFYUI_WORKFLOW_INPUT_NODE_ID", _section_get(c, "comfyui", "workflow_input_node_id", "1")))
    ...
    return Settings(
        ...
        nacos_service_name=_from_env("NACOS_SERVICE_NAME", _section_get(c, "nacos", "service_name", "image2psd-service")),
