...
def update_tokens_from_nacos(raw: str):
    ...
    with _nacos_lock:
        if new_tokens != _nacos_tokens:
            ...
            logger.info("Nacos token 热更新: %d 个 token", len(new_tokens))
    _refresh_allowed()
...
async def _nacos_poll_loop(service_name: str):
    logger.info("Nacos token 监听启动: dataId=%s interval=%ss", service_name, _NACOS_POLL_INTERVAL)
    while True:
        try:
...
async def start_nacos_token_watcher(service_name: str):
    ...
    if content is not None:
        ...
        logger.info("首次从 Nacos 加载 token 成功")
