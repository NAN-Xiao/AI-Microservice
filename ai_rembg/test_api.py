...
用法：
  ...
  python test_api.py D:\path\to\image.png http://127.0.0.1:9005
  python test_api.py D:\path\to\image.png http://127.0.0.1:9005 your-token
"""
...
def main() -> int:
    ...
    url = base_url.rstrip("/") + "/api/ai-rembg/remove-background"
    headers = {}
    if token:
        headers["Authorization"] = f"Bearer {token}"

    suffix = image_path.suffix.lower() or ".png"
    output_path = image_path.with_name(f"{image_path.stem}_removebg.png")

    print(f"[INFO] 请求地址: {url}")
    print(f"[INFO] 输入图片: {image_path}")
    print(f"[INFO] 输出文件: {output_path}")
    print("[INFO] 开始上传并等待 PNG 返回...")
