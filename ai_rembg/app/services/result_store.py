...
async def cleanup_remote_resources(data: dict[str, Any]) -> None:
    ...
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            ...
            if prompt_id:
                await client.post(f"{base_url}/history", json={"delete": [prompt_id]})
...
async def _delete_remote_file(client: httpx.AsyncClient, base_url: str, item: dict[str, Any]) -> None:
    ...
    payload = {"filename": filename, "subfolder": subfolder, "type": file_type}
    ...
    for path in ("/delete", "/api/delete"):
        try:
            ...
            if response.status_code >= 400:
                logger.warning(
                    "删除 ComfyUI 文件失败: path=%s status=%s body=%s payload=%s",
                    path,
...
async def _delete_remote_directory(client: httpx.AsyncClient, base_url: str, item: dict[str, Any]) -> None:
    ...
    payload = {"subfolder": subfolder, "type": dir_type}

    for path in ("/ai-rembg/delete-directory", "/api/ai-rembg/delete-directory", "/delete-directory", "/api/delete-directory"):
        try:
            response = await client.post(f"{base_url}{path}", json=payload)
            if response.status_code in (404, 405):
                continue
            if response.status_code >= 400:
                logger.warning(
                    "删除 ComfyUI 目录失败: path=%s status=%s body=%s payload=%s",
                    path,
                    response.status_code,
                    response.text[:500],
                    payload,
            ...
            return
    logger.warning("ComfyUI 目录删除接口不可用，请确认 cleanup delete route 已安装并重启 ComfyUI: payload=%s", payload)
