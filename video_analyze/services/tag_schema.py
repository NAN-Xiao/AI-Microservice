...
def update_tag_schema(new_tags: dict) -> dict:
    ...
    errors = _validate_schema_structure(new_tags)
    if errors:
...
def _validate_schema_structure(tags: dict) -> list[str]:
...
def sanitize_tags(raw: dict) -> tuple[dict, list[str]]:
    """
    用 video_tags.json 清洗 LLM 输出的标签。
    - 删除不在 schema 中的一级/二级 key
    - 删除不在候选列表中的标签值
