...
3. 环境变量覆盖：HOST、PORT、DEBUG、LLM_*、UPLOAD_DIR
"""
...
_LOCAL_PATH = Path(os.environ.get("VIDEO_ANALYZE_CONFIG", str(_ROOT / "settings.yaml")))
...
def _storage_get(cfg: dict[str, Any], key: str, default: Any) -> Any:
...
def _resolve_upload_dir(cfg: dict[str, Any]) -> Path:
    if "UPLOAD_DIR" in os.environ:
        return Path(os.environ["UPLOAD_DIR"]).expanduser()
    raw = _storage_get(cfg, "upload_dir", "tmp_uploads")
